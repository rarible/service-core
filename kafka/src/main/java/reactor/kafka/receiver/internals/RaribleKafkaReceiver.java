...
public class RaribleKafkaReceiver<K, V> implements KafkaReceiver<K, V> {
    ...
    public RaribleKafkaReceiver(ConsumerFactory consumerFactory, ReceiverOptions<K, V> receiverOptions) {
    ...
    public Flux<List<ReceiverRecord<K, V>>> receiveBatch(Integer prefetch) {
        return withHandler(AckMode.MANUAL_ACK, (scheduler, handler) -> handler
                .receive()
                .publishOn(scheduler, preparePublishOnQueueSize(prefetch))
                .map(records -> {
                            List<ReceiverRecord<K, V>> result = new ArrayList<>(records.count());
                            for (ConsumerRecord<K, V> record : records) {
                                result.add(new ReceiverRecord<>(
                                        record,
                                        handler.toCommittableOffset(record)));
                            }
                            return result;
